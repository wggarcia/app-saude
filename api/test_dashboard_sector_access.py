...
class DashboardSectorAccessTests(TestCase):
    ...
    def test_farmacia_so_acessa_ambiente_farmacia(self):
        ...
        self.assertContains(response, "SolusCRT Farmacia Intelligence")
    ...
    def test_hospital_so_acessa_ambiente_hospital(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "SolusCRT Hospital Command")
    ...
    def test_empresa_comum_nao_acessa_ambientes_setoriais(self):
        ...
        self.assertEqual(client.get("/dashboard-farmacia/").status_code, 302)
        self.assertEqual(client.get("/dashboard-farmacia/")["Location"], "/dashboard/")
        self.assertEqual(client.get("/dashboard-hospital/").status_code, 302)
        self.assertEqual(client.get("/dashboard-hospital/")["Location"], "/dashboard/")

        response = client.get("/dashboard/")
        self.assertEqual(response.status_code, 200)
        self.assertNotContains(response, "id=\"gotoFarmacia\"")
        self.assertNotContains(response, "id=\"gotoHospital\"")
