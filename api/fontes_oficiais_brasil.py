...
OPENDATASUS_DATASUS_MANIFEST = [
    ...
    },
    {
        ...
        "estrategia": "publicado apenas como .csv.zip por agravo/ano; coleta exige worker assincrono de descompactacao. Arboviroses ja cobertas em tempo real pela camada InfoDengue.",
        "risco_operacional": "medio_alto",
        "motivo_cuidado": "arquivos compactados (.csv.zip) incompativeis com amostragem por HTTP Range; campos variam por agravo",
        "fonte_exata_inicial": "https://opendatasus.saude.gov.br/dataset/arboviroses-dengue",
        "status": "requer_worker_descompactacao",
    },
    {
        "id": "vacinacao",
...
def _catalogo_opendatasus_datasus():
    ...
    return {
        ...
        "seguranca_download": {
            ...
            "sinan_agravos": "pendente: publicado como .csv.zip; requer worker de descompactacao (arboviroses ja cobertas pelo InfoDengue)",
            "vacinacao": "pendente: arquivos segmentados de grande volume; requer worker assincrono",
...
def panorama_brasil_oficial_payload(force=False):
    ...
    payload = {
        ...
        "lacunas_controladas": [
            "SINAN (agravos notificados) e publicado como .csv.zip e exige worker de descompactacao; arboviroses ja sao cobertas em tempo real pelo InfoDengue.",
            "Vacinacao (SI-PNI) e publicada em arquivos segmentados de grande volume e exige worker assincrono com janela operacional e governanca LGPD.",
