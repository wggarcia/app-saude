...
_PRODUTO_GOVERNO = (
    ...
    "- Vigilância epidemiológica EM TEMPO REAL. Um PAINEL ÚNICO mostra no mapa onde uma doença está "
    "crescendo AGORA no município, por bairro; e 9 sistemas de IA projetam PARA ONDE ela vai nos "
    "próximos 7, 14 e 30 dias. Assim a secretaria manda a equipe pro lugar certo ANTES da explosão de "
    "casos, em vez de descobrir semanas depois pelo SINAN. (Esse painel se chama 'Sala de Situação' — "
    "mas no email explique o que ele FAZ, não use só o nome técnico.)\n"
    "- Monitor global de notícias de saúde + alerta precoce; App Cidadão para alertar a população por bairro.\n"
    ...
    "\n"
    "COMO ESCREVER A ABERTURA (tem que ser IMPECÁVEL e CONCRETA):\n"
    "- Leads de SAÚDE ou PREFEITURA: NUNCA abra com 'um surto' genérico — o leitor não visualiza 'surto "
    "de quê'. Use uma doença CONCRETA e universal no Brasil, DENGUE é a melhor (todo município enfrenta "
    "todo ano). Modelo de abertura: 'Quando os casos de dengue começam a subir num bairro, sua secretaria "
    "fica sabendo em tempo real — ou só semanas depois, quando o caso já entrou no SINAN?'. Pode usar "
    "gripe/SRAG como segundo exemplo se fizer sentido.\n"
    "- NÃO escreva 'Sala de Situação' solta como se o leitor soubesse o que é. Descreva em linguagem "
    "simples: 'um painel que mostra, em tempo real, onde a doença está e para onde ela vai'. O nome "
    "técnico pode vir depois, entre parênteses.\n"
    "- Leads de ASSISTÊNCIA SOCIAL: siga o perfil do lead (abra pelo SUAS/CadÚnico/prestação de contas ao "
    "MDS) — NÃO fale de dengue nem de vigilância epidemiológica com eles.\n"
    "- Frases curtas, concretas, sem jargão de TI. Nada de 'solução', 'plataforma robusta', 'gestão 360'.\n"
    "\n"
    "COMO VENDER PRA GOVERNO: é CONTRATO ANUAL (não assinatura mensal), valor definido em reunião — "
